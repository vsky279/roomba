...
local TIME_ZONE = 3600 -- CET

local node_heap = node.heap
local function debug (fmt, ...)
  if (...) then fmt = fmt:format(...) end
  print("[ntp]", node_heap(), fmt)
end

return({
    tz=function(self, ts) -- UTC time as parameter
        return TIME_ZONE + (self:dst((ts or self:time()) + TIME_ZONE) and 3600 or 0)
    end,

    sync=function(self, cb, fail)
        sntp_sync({"0.cz.pool.ntp.org", "1.cz.pool.ntp.org", "2.cz.pool.ntp.org"},
            function (ts)
                debug("Time updated: "..self:format())
                if cb then cb(ts) end
            end, function ()
                debug("Time update fail!")
                if fail then fail() end
            end)
    end,

    ts2gmt=function(_, ts)
    ...
    format=function(self, ts)
        local dt=self:ts2gmt(ts or self:time())
